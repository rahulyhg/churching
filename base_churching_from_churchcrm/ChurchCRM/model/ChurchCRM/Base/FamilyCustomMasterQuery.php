...
abstract class FamilyCustomMasterQuery extends ModelCriteria
{
    ...
    {
        $sql = 'SELECT fam_custom_Order, fam_custom_Field, fam_custom_Name, fam_custom_Special, fam_custom_Side, fam_custom_FieldSec, type_ID FROM family_custom_master WHERE fam_custom_Field = :p0';
        try {
    ...
    public function prune($familyCustomMaster = null)
    {
        if ($familyCustomMaster) {
            $this->addUsingAlias(FamilyCustomMasterTableMap::COL_FAM_CUSTOM_FIELD, $familyCustomMaster->getCustomField(), Criteria::NOT_EQUAL);
        }
